...
async function* createDeployment(files, clientOptions, deploymentOptions) {
    ...
    try {
        const dpl = await utils_1.fetch(`${apiDeployments}${query_string_1.generateQueryString(clientOptions)}`, clientOptions.token, {
            method: 'POST',
            headers: {
...
async function* deploy(files, nowConfig, clientOptions, deploymentOptions) {
    const debug = utils_1.createDebug(clientOptions.debug);
    ...
    if (deploymentOptions.version === 1 &&
        !deploymentOptions.deploymentType &&
        nowConfig.type) {
        debug(`Setting 'type' for 1.0 deployment to '${nowConfig.type}'`);
        deploymentOptions.deploymentType = nowConfig.type.toUpperCase();
    }
    ...
    try {
        ...
        for await (const event of createDeployment(files, clientOptions, deploymentOptions)) {
